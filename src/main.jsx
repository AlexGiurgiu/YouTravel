...
const T = {
  en: {
    ...
    nav: { corporate: "Corporate Travel", family: "Bespoke Holidays", about: "About Us", contact: "Contact" },
    corp: {
      title: "Corporate Travel & Events",
      text: "From flights and hotels to team-building events, we optimize costs and provide 24/7 support so your team can focus on business.",
    ...
    fam: {
      ...
      text: "Family time is precious — holidays should be stress-free. We design escapes that fit your style, budget, and dreams.",
      b1: "Enjoy custom itineraries",
      b2: "Discover handpicked destinations",
      b3: "Benefit from the best price-quality balance",
    },
    about: { title: "Why Choose YouTravel?", text: "Because every journey is personal. We balance quality, cost, and reliability to make every trip effortless and memorable." },
    contact: { title: "Let’s Plan Your Next Journey", text: "Contact us today and we’ll take care of the rest:", email: "Email Us", phone: "Call Us", whatsapp: "WhatsApp" },
    backHome: "Back to Home",
  },
  ro: {
    headline: "Călătorii personalizate. Oriunde, oricând.",
    subheadline: "Călătorii inteligente pentru afaceri și familii.",
    nav: { corporate: "Corporate Travel", family: "Vacanțe Personalizate", about: "Despre noi", contact: "Contact" },
    corp: {
      title: "Călătorii & Evenimente Corporate",
      text: "De la zboruri și hoteluri la team building-uri, optimizăm costurile și oferim suport 24/7 pentru ca echipa ta să se concentreze pe business.",
    ...
    fam: {
      ...
      text: "Timpul cu familia este prețios — vacanțele trebuie să fie fără griji. Creăm experiențe adaptate stilului, bugetului și viselor voastre.",
      b1: "Bucură-te de itinerarii personalizate",
      b2: "Descoperă destinații selectate",
      b3: "Profită de cel mai bun raport calitate-preț",
    },
    about: { title: "De ce YouTravel?", text: "Pentru că fiecare călătorie este personală. Echilibrăm calitatea, costul și fiabilitatea pentru călătorii ușoare și memorabile." },
    contact: { title: "Planifică următoarea călătorie", text: "Contactați-ne și ne ocupăm de tot:", email: "Trimiteți Email", phone: "Sunați-ne", whatsapp: "WhatsApp" },
...
function Header({ t, switchLang, lang }) {
  ...
  return (
    <header className={`yt-header fixed top-0 left-0 w-full z-50 border-b border-neutral-200 ${scrolled ? "shadow-md" : "shadow-none"}`}>
      <div className="max-w-6xl mx-auto flex items-center justify-between py-3 px-4">
        <button onClick={() => scrollToId("hero")} className="flex items-center" aria-label="YouTravel">
          <img src={`${import.meta.env.BASE_URL}images/logo-youtravel-blue.png`} alt="YouTravel" className="h-12 w-auto" />
        </button>
        ...
        <nav className="hidden md:flex items-center gap-3">
          ...
          <div className="relative ml-2">
            ...
            >
              ...
              <svg width="14" height="14" viewBox="0 0 20 20" className="opacity-60"><path d="M5 7l5 6 5-6H5z" fill="currentColor"/></svg>
            </button>
            {openLang && (
              <div className="absolute right-0 mt-2 w-32 bg-white shadow-lg rounded-lg border border-gray-200 overflow-hidden z-50">
                <button onClick={() => { setOpenLang(false); switchLang("en"); }} className="block w-full text-left px-3 py-2 hover:bg-gray-50">EN</button>
                <button onClick={() => { setOpenLang(false); switchLang("ro"); }} className="block w-full text-left px-3 py-2 hover:bg-gray-50">RO</button>
              </div>
            )}
          </div>
        </nav>
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute right-0 top-0 h-full w-72 bg-white shadow-xl p-4 flex flex-col gap-2">
            ...
            <div className="border rounded-lg overflow-hidden">
              <button onClick={() => { scrollToId("corporate"); setMobileOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 border-b">{t.nav.corporate}</button>
              <button onClick={() => { scrollToId("family"); setMobileOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 border-b">{t.nav.family}</button>
              <button onClick={() => { scrollToId("about"); setMobileOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 border-b">{t.nav.about}</button>
              <button onClick={() => { scrollToId("contact"); setMobileOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50">{t.nav.contact}</button>
            </div>
            <div className="mt-3">
...
function Hero({ t }) {
  return (
    <section id="hero" className="h-[80vh] md:h-[92vh] relative">
      <img src={`${import.meta.env.BASE_URL}images/hero-skyline.jpg`} alt="Hero" className="absolute inset-0 w-full h-full object-cover" />
    ...
    <section id="corporate" className="bg-[#0F1F36] py-16 text-white">
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-10 items-center">
        ...
        <div className="grid grid-cols-2 gap-4">
          <img src={`${import.meta.env.BASE_URL}images/corp-skyline.jpg`} alt="Skyline" className="rounded-xl shadow-lg object-cover w-full h-44 md:h-56" />
          <img src={`${import.meta.env.BASE_URL}images/corp-rooftop.jpg`} alt="Rooftop" className="rounded-xl shadow-lg object-cover w-full h-44 md:h-56" />
          <img src={`${import.meta.env.BASE_URL}images/corp-night.jpg`} alt="Night City" className="hidden md:block rounded-xl shadow-lg object-cover w-full h-44 md:h-56 col-span-2" />
        </div>
      </div>
    </section>
  );
    ...
    <section id="family" className="yt-peach py-16">
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-10 items-center">
        ...
        <div className="order-2 grid grid-cols-2 gap-4">
          <img src={`${import.meta.env.BASE_URL}images/fam-pineapple.jpg`} alt="Tropical pineapple drink on the beach" className="rounded-xl shadow-lg object-cover w-full h-44 md:h-56" />
          <img src={`${import.meta.env.BASE_URL}images/fam-airplane-hand.jpeg`} alt="Hand by airplane window" className="rounded-xl shadow-lg object-cover w-full h-44 md:h-56" />
          <img src={`${import.meta.env.BASE_URL}images/fam-jetski.jpg`} alt="Family riding a jetski" className="hidden md:block rounded-xl shadow-lg object-cover w-full h-44 md:h-56 col-span-2" />
        </div>
      </div>
    </section>
  );
    ...
    <section id="about" className="yt-white py-16">
      <div className="max-w-6xl mx-auto px-6 text-center">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <img src={`${import.meta.env.BASE_URL}images/about-centralpark.jpg`} alt="Central Park" className="hidden md:block rounded-xl shadow-md object-cover h-56 md:h-64 w-full" />
          <img src={`${import.meta.env.BASE_URL}images/about-eiffel.jpg`} alt="Eiffel Tower" className="hidden md:block rounded-xl shadow-md object-cover h-56 md:h-64 w-full" />
          <img src={`${import.meta.env.BASE_URL}images/about-canyon.jpg`} alt="Canyon" className="rounded-xl shadow-md object-cover h-56 md:h-64 w-full" />
        </div>
      </div>
    </section>
  );
...
function Contact({ t }) {
  const BTN =
    "inline-flex items-center justify-center gap-2 bg-[#E76F51] text-white px-6 py-3 rounded-lg shadow hover:bg-[#d65b42] focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#E76F51] focus-visible:ring-offset-[#1D3557]";
    ...
    <section id="contact" className="yt-navy py-16 text-white">
      <div className="max-w-6xl mx-auto px-6 text-center">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
          <a href="mailto:office@youtravel.ro" className={BTN}>📧 {t.contact.email}: <span className="font-semibold">office@youtravel.ro</span></a>
          <a href="tel:+40720377378" className={BTN}>📞 {t.contact.phone}: <span className="font-semibold">+40 720 377 378</span></a>
          <a href="https://wa.me/40720377378" target="_blank" rel="noopener noreferrer" className={BTN}>💬 {t.contact.whatsapp}</a>
        </div>
      </div>
    </section>
  );
    ...
    <footer className="bg-gray-900 text-white py-6">
      <div className="max-w-6xl mx-auto px-6 flex flex-col items-center gap-4 text-center">
        <div className="flex items-center gap-4">
          ...
          <Link to={path} className="text-xs underline opacity-80 hover:opacity-100">
